...
let boutonSuivant = document.getElementById('suivant') as HTMLButtonElement;
let boutonPrecedent = document.getElementById('precedent') as HTMLButtonElement;
...
function validerEtape(etape: number): boolean {
    ...
    switch (etape) {
        case 0:
            ...
            const dureeRadios = document.querySelectorAll('input[name="duree"]') as NodeListOf<HTMLInputElement>;
            const erreurFrequence = document.getElementById('erreur-duree') as HTMLElement;
            let dureeValide = false;

            dureeRadios.forEach(radio => {
                if (radio.checked) dureeValide = true;
            });
            ...
            const montantRadios = document.querySelectorAll('input[name="montant"]')as NodeListOf<HTMLInputElement>;
            const autreMontant = document.getElementById('facultatif') as HTMLInputElement;
            const erreurMontant = document.getElementById('erreur-montant') as HTMLElement;
